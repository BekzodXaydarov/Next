...
const SearchManufacturer = ({manufacturer,setManufacturer} : SearchManufacturerProps) => {
  ...
  const filterManufacturers = query === "" ? manufacturers : manufacturers.filter((item: string)=>(
                     ...
                     <Combobox.Options>
                        {filterManufacturers.length === 0 && query !== "" ? (
                            ...
                            filterManufacturers.map((item)=>(
                                <Combobox.Option key={item} className={({active}:any)=>`relative search-manufacturer__option ${active ? "bg-primary-blue text-white":"text-gray-900"}`} value={item}>
                                     {({selected,active})=>{
                                      return <>
